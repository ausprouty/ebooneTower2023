...
function displayTeacherNotes() {
  ...
  if (trainerNotesHidden.length == 0 && trainerNotesVisible.length == 0) {
    ...
    var showNotes = ''
    if (localStorage.getItem('launch-teaching-notes')) {
      showNotes = localStorage.getItem('launch-teaching-notes')
    }
    if (showNotes == '') {
      hideTeacherNotesToggle()
    }
...
function toggleTeacherNotes() {
  var showNotes = 'show'
  if (localStorage.getItem('launch-teaching-notes')) {
    if (localStorage.getItem('launch-teaching-notes') == 'show') {
      showNotes = 'hide'
    }
  }
  localStorage.setItem('launch-teaching-notes', showNotes)
  displayTeacherNotes()
}
