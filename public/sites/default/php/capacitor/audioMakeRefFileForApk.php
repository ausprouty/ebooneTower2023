...
function audioMakeRefFileForApk($p){
   $output = '';
   $series_audios = [];
   $chapter_audios = [];
 //find series data that has been prototyped
        ...
        foreach ($text->chapters as $chapter){
            if (isset($chapter->prototype)){
                if ($chapter->prototype){
                    $chapter_audios = audioFindForApk($p, $chapter->filename);
                ...
                writeLogError('audioMakeRefFileForApk-36', $chapter);
            }
    ...
    audioMakeBatFileForApkWrite($output, $p);

    return $output;
}

function audioMakeBatFileForApkWrite($text, $p){
...
/*Input is:
    ...
    <hr /></div>';
*/
function audioFindForApk($p, $filename){
    //writeLog('audioFindForApk-113-p', $p);
    //writeLog('audioFindForApk-114-filename', $filename);
    // find chapter that has been prototyped
    $chapter_audios = [];
    $audio = [];
    $audio['filename'] =$filename;
    $new_name= audioFindForApkNewName($filename);
    ...
    $data = sqlArray($sql);
    $text= $data['text'];
    //writeLog('audioFindForApk-76-'. $filename, $text);
    ...
    for ($i = 0; $i < $count; $i++){
        ...
        //writeLog('audioFindForApk-95-'. $filename . $count, $url . "\n" . $find);
        $audio['url']= $url;
    ...
    //writeLog('audioFindForApk-185-chapteraudios', $chapter_audios);
    return $chapter_audios;
}
function  audioFindForApkNewName($filename){
   return $filename;
}
