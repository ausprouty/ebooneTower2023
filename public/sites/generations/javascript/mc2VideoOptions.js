...
function mc2DisplayPrimaryVideoOptions(div, video_type = "all"){
    ...
    console.log (data);
    // select language
    var title_text = '<div class="alert"><form onsubmit="mc2SavePrimaryVideoOption();">' + "\n" + '<h3>' + data.select_language[language] + '</h3>'+ "\n";
...
function mc2DisplayAlternativeVideoOption(div){
	...
	var language = mc2GetDisplayLanguage();
    var data = localVideoData();
	var form_text = '<div class="alert"><form onsubmit="mc2SaveAlternateVideoOption();">' + "\n";
...
function mc2SaveAlternateVideoOption(){
    if (document.getElementById("yes").checked){
		var data = localVideoData();
        localSetPreference("mc2AlternativeVideoLanguage", data.default_video_language);
    }
    else{
...
function mc2CreateIframeArclight(video){
	...
	var data = localVideoData();
    var language = mc2GetDisplayLanguage();
	if (data.allow_user_to_select_language == "true" && mc2MoreThanOneLanguage(video)){
