document.addEventListener('DOMContentLoaded', noteTest)
function noteTest(){
    let db = new Localbase('db')

    db.collection('notes').doc('mykey-1').set({
        note: 'Bill',
      })
...
function getNote(key){
    let db = new Localbase('db')
    db.collection('notes').doc(key).get().then(document => {
        console.log(document)
        return document
    })
