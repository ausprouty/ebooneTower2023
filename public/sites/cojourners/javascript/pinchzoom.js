;(function (e) {
  if ('function' == typeof bootstrap) bootstrap('pinchzoom', e)
  ...
  })(
    {
      ...
      3: [
        function (require, module, exports) {
          ...
          module.exports = require('./track-pointer-events')
            ? window.navigator.msMaxTouchPoints &&
              window.navigator.msMaxTouchPoints > 1
            : require('./track-touch-events') &&
              document.implementation.hasFeature(
                'http://www.w3.org/TR/SVG11/feature#BasicStructure',
                '1.1'
              )
        },
        { './track-pointer-events': 5, './track-touch-events': 6 },
      ],
      4: [
        function (require, module, exports) {
          /**
          ...
          function PinchZoom(image, containerNode, options) {
            ...
            if (!(this instanceof PinchZoom))
              return new PinchZoom(image, containerNode, options)

            if (!(image instanceof HTMLImageElement) || !image.parentNode) {
              throw new TypeError(
                'PinchZoom requires an Image node which is inserted in the DOM'
              )
            }

            this.config = require('./defaults')(defaultConfig, options)
            if (
              !this.config.zoomControlText ||
              !this.config.zoomControlText.zoomin ||
              !this.config.zoomControlText.zoomout
            ) {
              throw new TypeError(
                'zoomControlText must be an object in the form {zoomin: "+", zoomout: "-"}.'
              )
            }
            ...
            this.cssTransitionTimingFunction =
              cssPrefix.transition + 'TimingFunction'
          ...
          PinchZoom.prototype.onTouchStart = function (event) {
            ...
            for (i = 0, l = event.targetTouches.length; i < l; i++) {
              ...
              if (
                this.activeInputs.length >= 2 ||
                this.activeInputs[eachTouch.identifier] !== undefined
              ) {
                continue
              }
          ...
          PinchZoom.prototype.onTouchEnd = function (event) {
            ...
            for (i in this.activeInputs) {
              if (this.activeInputs.hasOwnProperty(i)) {
                ...
                if (
                  typeof eachTouch !== 'object' ||
                  eachTouch.lastX === false
                ) {
                  continue
                }
          ...
          PinchZoom.prototype.onMouseWheel = function (event) {
            ...
            this.mouseWheelThrottle = window.setTimeout(
          ...
          PinchZoom.prototype.onPointerMove = function (event) {
            ...
            if (this.allowClickEvent) {
              if (
                Math.abs(trackedTouch.originX - trackedTouch.lastX) > 2 ||
                Math.abs(trackedTouch.originY - trackedTouch.lastY) > 2
              ) {
                this.allowClickEvent = false
              }
            }
          ...
          PinchZoom.prototype.updateDimensions = function () {
            ...
            this.scale = Math.min(
              1.0,
              this.containerDimensions.w / this.imageDimensions.w,
              this.containerDimensions.h / this.imageDimensions.h
            )
          ...
          PinchZoom.prototype.updatePosition = function () {
            ...
            x +=
              (this.containerDimensions.w -
                this.imageDimensions.w * this.scale) /
              2
            y +=
              (this.containerDimensions.h -
                this.imageDimensions.h * this.scale) /
              2

            // Amend with the current scale factor and round to nearest pixel
            x =
              Math.round((x / this.scale) * this.roundFactor) / this.roundFactor
            y =
              Math.round((y / this.scale) * this.roundFactor) / this.roundFactor

            // Render
            this.imageStyle[this.cssTransform] =
              'scale(' + this.scale + ') translate3d(' + x + 'px,' + y + 'px,0)'
          }

          PinchZoom.prototype.updatePositionWithAnimationDuration =
            function () {
              var self = this

              this.imageStyle[this.cssTransitionDuration] =
          ...
          PinchZoom.prototype.processInputMove = function () {
            ...
            if (this.activeInputs.length === 2) {
              ...
              var originalDistance = Math.sqrt(
                Math.pow(e2.originX - e1.originX, 2) +
                  Math.pow(e2.originY - e1.originY, 2)
              )
              var newDistance = Math.sqrt(
                Math.pow(e2.lastX - e1.lastX, 2) +
                  Math.pow(e2.lastY - e1.lastY, 2)
              )
            ...
            for (k in this.activeInputs) {
              if (this.activeInputs.hasOwnProperty(k)) {
                ...
                totalX +=
                  this.activeInputs[k].lastX - this.activeInputs[k].originX
                totalY +=
                  this.activeInputs[k].lastY - this.activeInputs[k].originY
              }
            }
            this.offset.x =
              this.offset.savedX + totalX / this.activeInputs.length
            this.offset.y =
              this.offset.savedY + totalY / this.activeInputs.length
            this.updatePosition()
          }
          ...
          PinchZoom.prototype.processInputEnd = function () {
            ...
            if (targetScale > this.config.maxScale)
              targetScale = this.config.maxScale
            ...
            if (
              this.imageDimensions.w * targetScale <=
              this.containerDimensions.w
            ) {
              this.offset.x = 0
            } else if (pos.containerX + pos.offsetX > pos.imageX) {
              this.offset.x = Math.round(
                (pos.imageX - pos.containerX) * targetScale
              )
            } else if (pos.containerX > pos.offsetX + pos.imageX) {
              this.offset.x = Math.round(
                (pos.containerX - pos.imageX) * targetScale
              )
            }

            // Do the same for height
            if (
              this.imageDimensions.h * targetScale <=
              this.containerDimensions.h
            ) {
              this.offset.y = 0
            } else if (pos.containerY + pos.offsetY > pos.imageY) {
              this.offset.y = Math.round(
                (pos.imageY - pos.containerY) * targetScale
              )
            } else if (pos.containerY > pos.offsetY + pos.imageY) {
              this.offset.y = Math.round(
                (pos.containerY - pos.imageY) * targetScale
              )
            }
          ...
          PinchZoom.prototype.stepZoom = function (direction) {
            if (direction === 'out') {
              this.scale = Math.max(
                ...
                this.scale - this.config.stepZoomIncrement
              )
            } else {
              this.scale = Math.min(
          ...
          PinchZoom.prototype.addControlsTo = function (anEle) {
            var singleZoomControlClass =
              this.config.zoomControlClass +
              ' ' +
              this.config.zoomControlClass +
              '_zoom'

            if (!anEle || !anEle.appendChild)
              throw new TypeError('addControlsTo requires a valid DOM node')
            ...
            if (!this.zoomControls) {
              ...
              if (this.trackPointerEvents || this.multitouchSupport) {
                ...
                if (this.config.zoomControlMessage)
                  this.zoomControls.innerHTML =
                    '<div class="' +
                    this.config.zoomControlClass +
                    '_message">' +
                    this.config.zoomControlMessage +
                    '</div>'
              } else {
                this.zoomControls.innerHTML =
                  '<button class="' +
                  singleZoomControlClass +
                  'out">' +
                  this.config.zoomControlText.zoomout +
                  '</button>' +
                  '<button class="' +
                  ...
                  '</button>'
                this.zoomControls
                  .getElementsByClassName(singleZoomControlClass + 'out')[0]
                  .addEventListener(
                    'click',
                    this.onStepZoomClick.bind(this, 'out'),
                    false
                  )
                this.zoomControls
                  .getElementsByClassName(singleZoomControlClass + 'in')[0]
                  .addEventListener(
                    'click',
                    this.onStepZoomClick.bind(this, 'in'),
                    false
                  )
              }
            }
        ...
        },
        {
          './defaults': 1,
          './event-listeners': 2,
          './multitouch-support': 3,
          './track-pointer-events': 5,
          './vendor-css-prefix': 7,
        },
      ],
      5: [
        ...
        function (require, module, exports) {
          var prefix =
            ...
            Object.prototype.toString.call(window.opera) === '[object Opera]'
              ? 'o'
              : document.documentElement.style.hasOwnProperty('MozAppearance')
              ? 'Moz'
              : document.documentElement.style.hasOwnProperty(
                  'WebkitAppearance'
                )
              ? 'webkit'
